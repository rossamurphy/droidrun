...
class JeevesIntegration:
    ...
    async def _get_ui_state_via_broadcast(self, device, start_time: float) -> Dict[str, Any]:
        """
        Get UI state via broadcast response - FASTEST method!
        No ContentProvider query overhead, no file I/O
        """
        try:
            # Start listening for the response broadcast BEFORE sending request
            # This uses a simple polling approach since ADB doesn't have great broadcast listening
            ...
            # Poll for the response using logcat (Jeeves logs when it sends broadcasts)
            max_retries = 20  # 20 * 50ms = 1 second max wait
            for attempt in range(max_retries):
                await asyncio.sleep(0.05)  # 50ms polling interval
                
                # Check logcat for the broadcast response log
                result = await device._adb.shell(
                    device._serial,
                    "logcat -d -t 10 | grep 'UI state broadcast sent'"
                )
                
                if result and "UI state broadcast sent" in result:
                    # Jeeves sent the broadcast, now try to capture it via dumpsys
                    # or use ContentProvider as fallback for now
                    elapsed = time.time() - start_time
                    
                    logger.info(f"📡 Jeeves broadcast detected in {elapsed:.3f}s, using ContentProvider for data")
                    
                    # For now, fall back to ContentProvider for actual data
                    # TODO: Implement proper broadcast listening
                    cp_result = await device._adb.shell(
                        device._serial,
                        f"content query --uri {self.content_provider_uri}"
                    )
                    
                    if cp_result and cp_result.strip():
                        ui_state = self._parse_content_provider_response(cp_result.strip())
                        total_elapsed = time.time() - start_time
                        
                        logger.info(f"🚀 Jeeves broadcast + ContentProvider: {len(ui_state.get('a11y_tree', []))} elements in {total_elapsed:.3f}s")
                        
                        # Mark this as broadcast method
                        ui_state["source"] = "jeeves_broadcast_hybrid"
                        ui_state["method"] = "broadcast_trigger_contentprovider_data"
                        
                        return ui_state
            
            # If we get here, no broadcast was detected
            logger.warning("⚠️ No broadcast response detected, falling back to ContentProvider")
