...
class ADBWrapper:
    ...
    async def _run_command(
        self, args: List[str], timeout: Optional[float] = None, check: bool = True
    ) -> Tuple[str, str]:
        """Run an ADB command.
        ...
        try:
            ...
            raise TimeoutError(f"ADB command timed out: {' '.join(cmd)}") from None
        except FileNotFoundError:
            raise FileNotFoundError(f"ADB not found at {self.adb_path}") from None

    async def _run_device_command(
        self, serial: str, args: List[str], timeout: Optional[float] = None, check: bool = True
    ) -> Tuple[str, str]:
    ...
    async def pull_file(self, serial: str, device_path: str, local_path: str) -> Tuple[str, str]:
        ...
        return await self._run_device_command(
            serial, ["pull", device_path, local_path], timeout=60.0
        )
